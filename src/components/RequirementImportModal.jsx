...
import {
  parseRequirementExcelFile,
  validateRequirements,
  convertExcelToRequirements,
  downloadRequirementExcelTemplate,
} from '../utils/requirementExcelImport';

/**
 * 요구사항 엑셀 가져오기 모달
 */
export default function RequirementImportModal({ isOpen, onClose, onImport, selectedProject }) {
  ...
  const handleFileSelect = async (e) => {
    ...
    try {
      const data = await parseRequirementExcelFile(selectedFile);
      setParsedData(data);
  ...
  const handleImport = () => {
    ...
    const requirements = convertExcelToRequirements(validationResult.valid, selectedProject.id);
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="엑셀에서 요구사항 가져오기">
      <div>
        {/* 템플릿 다운로드 */}
          ...
          <div className="flex items-start">
            ...
            <div className="flex-1">
              ...
              <Button
                type="button"
                variant="primary"
                onClick={downloadRequirementExcelTemplate}
